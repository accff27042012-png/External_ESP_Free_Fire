...
// ============================================================
// DUMMY DATA - TĂNG DUNG LƯỢNG (~500KB)
// ============================================================
static const unsigned char dummyData[524288] = {
    #include "dummy.inc"  // Nếu có file này, hoặc dùng define bên dưới
};

// Thay vì dùng file, tao tạo mảng bằng define
#define DUMMY_SIZE 524288
static char dummyBuffer[DUMMY_SIZE];
...
__attribute__((used)) static void dummyFunction1() {
    for (int i = 0; i < 10000; i++) {
        dummyBuffer[i % DUMMY_SIZE] = (char)(i & 0xFF);
        dummyBuffer[(i + 5000) % DUMMY_SIZE] ^= (char)(i >> 8);
    }
}
...
__attribute__((used)) static void dummyFunction2() {
    ...
    NSLog(@"[Dummy] Hash: %u", hash);
}

__attribute__((used)) static void dummyFunction3() {
    float arr[1024];
    for (int i = 0; i < 1024; i++) {
        arr[i] = sinf(i * 0.1f) * cosf(i * 0.05f);
        arr[i] += tanhf(i * 0.01f);
    }
    for (int i = 0; i < 512; i++) {
...
__attribute__((used)) static void dummyFunction4() {
    ...
    str[4095] = '\0';
    unsigned long crc = crc32(0, (const Bytef*)str, 4095);
    NSLog(@"[Dummy] CRC: %lu", crc);
}
...
static void createMenuOnWindow(UIWindow *window) {
    ...
    // Gọi dummy functions để compiler không tối ưu hóa
    dummyFunction1();
    dummyFunction2();
    dummyFunction3();
    dummyFunction4();
    ...
    [NSTimer scheduledTimerWithTimeInterval:0.05 repeats:YES block:^(NSTimer *timer) {
        ...
        dummyBuffer[arc4random() % DUMMY_SIZE] = (char)(arc4random() & 0xFF);
    }];
    
    NSLog(@"[HungVn] Menu created! Size: ~1MB");
}
...
__attribute__((constructor)) static void init() {
    ...
    // Khởi tạo dummy buffer
    for (int i = 0; i < DUMMY_SIZE; i++) {
        dummyBuffer[i] = (char)(i & 0xFF);
    }
